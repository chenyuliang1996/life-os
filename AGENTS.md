...
<life_os_project_profile>
## Life OS Project Profile

Product intent:
- Build for real users, not demo narratives.
- Keep ToC interaction focused on user goals (plan, confirm, execute, review).
- Keep ToB views focused on observability and operational confidence.
- Preserve bilingual behavior (`zh` / `en`) across all user-facing changes.

Architecture boundaries:
- `life-os-web`: Spring Boot API, H5 pages, AG-UI runtime agent.
- `life-os-orchestrator`: main orchestration and confirmation flow.
- `life-os-agents`: travel, learning, schedule specialist agents.
- `life-os-tools`: FlyAI/A2A/external connector adapters.
- `life-os-memory`: short-term and long-term memory facade.
- `life-os-rag`: retrieval and vector store abstraction.
- `life-os-infra`: persistence implementation and DB adapters.
- `life-os-domain`: core domain model and interfaces.

Delivery constraints:
- Do not break existing REST contracts without updating docs and callers.
- Do not introduce user-facing copy with "demo", "mock", or "prototype".
...
- Test: `mvn test`
- Run: `mvn -f life-os-web/pom.xml spring-boot:run`
- Smoke URLs:
  - `http://127.0.0.1:8080/`
  - `http://127.0.0.1:8080/en/index.html`
  - `http://127.0.0.1:8080/?surface=tob`
</life_os_project_profile>
